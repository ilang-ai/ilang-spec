# I-Lang Protocol Specification v4.0 RC2

```
[PROTOCOL:I-Lang|v=4.0|type=release-candidate|status=RC2]
[PROTOCOL:I-Lang|v=4.0|fallback=3.0|degrade=warn|unsafe=safe_mode]
[AUTHOR:@SUN|role=discoverer]
[CO-AUTHOR:@BRO|role=co-designer]
[RED-TEAM:@GPT-5.5-Pro|role=审查|rounds=2]
[LICENSE:MIT]
```

> v3.0 = communication format. v4.0 = execution semantics.
> RC2 resolves all 5 freeze blockers from RC1 red-team review.
...
::FALLBACK{unsupported_exec_semantics⇒safe_mode}
...
Current environment may not enforce ::UNTRUSTED, STATUS authority,
BUDGET, or external grading. Processing in read-only safe-mode.
```
...
- `::DATA{id:u1|trust:untrusted|source:user|interpret:payload_only}` is inline short form for metadata-only reference (payload stored externally)
...
::STATUS{@TASK|state:stopped|reason:budget|progress:60%|next:resume_step_4|by:@RUNTIME}
::STATUS{@TASK|state:stopped|reason:user_pause|by:@RUNTIME}
::STATUS{@TASK|state:blocked|need:api_key|by:@AGENT}
::STATUS{@TASK|state:failed|reason:unrecoverable|detail:...|by:@AGENT}
::STATUS{@TASK|state:needs_revision|missing:d3,d4|score:0.78|by:@GRADER}
```
...
```
[CHEK:@OBJECTIVE|extract=deliverables]
...
::STATUS{@TASK|state=claimed_complete|evidence=@AUDIT_REPORT|by=@SELF}
  → if ALL evidence pass and score >= threshold
::STATUS{@TASK|state=needs_revision|missing=gaps|by=@SELF}
...
1. `::LEDGER{}` for work-already-done tracking (prevents repeat work in long tasks). Candidate for v4.1.
2. `::TRACE{}` / `::EVENT{}` for transcript recording. v3 LOG verb exists but v4 may need structured trace. Candidate for v4.1.
3. `::MEMORY_POLICY{}` for controlling agent memory writes. Candidate for v4.1.
4. `::PERMIT{}` for side-effect/permission gating. Candidate for v4.1.
5. npm @i-language/spec@4.0.0 JSON schema for new declarations.
6. Multi-agent STATUS conflict resolution with `::LOCK{}`.

---

```
[PROTOCOL:I-Lang|v=4.0|status=RC2]
...
warn-open for communication, fail-safe for execution.
Red-team reviewed (2 rounds). Not yet frozen.
```
